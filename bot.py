...
# ═══════════════════════════════════════════════
#  CONFIGURACIÓN — edita esto antes de arrancar
# ═══════════════════════════════════════════════
BOT_TOKEN    = os.environ.get("BOT_TOKEN")       # Token de BotFather
ADMIN_ID     = 8315702370             # Tu ID (búscalo con @userinfobot)
SUPPORT_USER = "@ACPremiumSupport"   # Tu usuario de soporte

# Links de invitación de tus canales (genera uno en cada canal)
CHANNEL_LINKS = {
...
# Planes con precios y duración en días
PLANES = {
...
# Cupones de descuento: {codigo: porcentaje}
CUPONES = {
    "NEW20": 20,
}
...
def get_suscripcion(user_id: int):
    db = cargar_db()
    return db.get(str(user_id))

def guardar_suscripcion(user_id: int, plan: str, dias: int):
    db = cargar_db()
    inicio = datetime.now()
...
# Estado temporal: {user_id: {"plan": ..., "cupon": ...}}
pendientes = {}

# Menú persistente (barra inferior)
...
def precio_con_descuento(precio: int, cupon: str) -> tuple[int, int]:
    """Devuelve (precio_final, descuento_%)"""
    pct = CUPONES.get(cupon.upper(), 0)
    return int(precio * (1 - pct / 100)), pct

def estado_suscripcion_texto(user_id: int) -> str:
    ...
    barra = min(int((1 - restante.total_seconds() /
                     (timedelta(days=PLANES[sub["plan"]]["dias"]).total_seconds())) * 10), 10)
...
async def ver_precios_msg(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Desde botón del menú de teclado"""
...
async def mi_suscripcion_msg(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    texto = estado_suscripcion_texto(user_id)
...
async def mostrar_pago(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    partes = query.data.split("_")   # pago_mensual_nocupon  o  pago_mensual_NEW20
    plan   = partes[1]
...
async def aprobar(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        ...
        return
    if not ctx.args:
        await update.message.reply_text("Uso: `/aprobar USER_ID` o `/aprobar USER_ID plan`",
                                        parse_mode="Markdown")
        return
    ...
    await update.message.reply_text(
        f"✅ Aprobado. Plan *{plan}* activado para `{user_id}` hasta {fin}.",
        parse_mode="Markdown"
    )
...
async def ver_pendientes(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for uid, datos in pendientes.items():
        texto += f"• `{uid}` — Plan *{datos['plan']}*"
        if datos.get("cupon"):
            texto += f" (cupón {datos['cupon']})"
        texto += "\n"
    await update.message.reply_text(texto, parse_mode="Markdown")


# ── /suscripciones (admin: ver todas) ────────────────────────
async def ver_suscripciones(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not db:
        ...
        return
    ahora = datetime.now()
    texto = "📊 *Todas las suscripciones:*\n\n"
    for uid, sub in db.items():
        fin = datetime.fromisoformat(sub["fin"])
        estado = "✅" if fin > ahora else "❌"
        dias_rest = max((fin - ahora).days, 0)
        texto += f"{estado} `{uid}` — *{sub['plan']}* — {dias_rest}d restantes\n"
    await update.message.reply_text(texto, parse_mode="Markdown")
